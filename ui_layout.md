...
| 격자 on/off, 스냅 | **4-5** | ✅ |
| 레일 9+1 · 플라이아웃 stub 맵 | **5단계** | ✅ |
| 테마 토글 | **12-1** | ✅ |
| 단축키 골격 | **5-3** / **12-3** | ✅ 골격 / ⬜ 도구별 완성 |
| SVG 아이콘 (핵심 UI + stub) | **12-5** | ✅ [`icon_guidelines.md`](icon_guidelines.md) |
| 저장·헤더 | **12-4**, 11-3 | ⬜ |

전체 도구: [`tool_catalog.md`](tool_catalog.md)

**5단계 JS 추가**: `isToolStub`, `showRailOnlyNotice`, `ALGEO_TOOL_KEY_MAP`, stub 배지 렌더

---

## 사용법 (신 UI)

1. **좌측 레일**에서 카테고리(점·원·선 등) 클릭 — 원본 순서
2. **플라이아웃**에서 세부 도구 선택 (`준비` 배지 = stub)
3. 캔버스에서 작도 (stub은 가이드만, 클릭 무시)
4. 블록코딩·설정 레일 = 이후 단계 안내
4. **우측 `+` `−` `⌂`** 으로 줌·원점 이동
